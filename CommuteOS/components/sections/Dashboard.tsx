          ...
          <div className="p-5 grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="lg:col-span-2 flex flex-col gap-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {kpis.map((kpi) => {
                  ...
                  return (
                    <div
                      key={kpi.label}
                      className="bg-[#111] border border-[#1a1a1a] rounded-lg p-3.5"
                    >
                      ...
                      <div className="text-lg font-semibold text-[#e8e8e8] tracking-tight tabular-nums">
                        {kpi.value}
                      </div>
              ...
              <div className="bg-[#111] border border-[#1a1a1a] rounded-lg p-4">
                    ...
                    <div key={i} className="flex-1 flex flex-col items-center gap-1">
                      <div
                        ...
                        style={{
                          ...
                          background:
                            val > 85
                              ? "rgba(111,207,115,0.6)"
                              : val > 70
                              ? "rgba(111,207,115,0.3)"
                ...
                <div className="flex gap-1.5 mt-2">
                  {weekDays.map((d, i) => (
                    <div key={i} className="flex-1 text-center text-2xs text-[#333]">
                      {d[0]}
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-[#111] border border-[#1a1a1a] rounded-lg p-4">
                ...
                <div className="flex gap-4 mt-3 text-2xs text-[#444]">
                  <span className="flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-sm bg-[rgba(111,207,115,0.7)]" />
                    High demand
                  </span>
                  <span className="flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-sm bg-[rgba(111,207,115,0.2)]" />
                    Medium
                  </span>
                  <span className="flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-sm bg-[rgba(255,255,255,0.05)]" />
                    Low
                  </span>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-3">
              <div className="bg-[#111] border border-[#1a1a1a] rounded-lg p-4 flex-1">
                ...
                <div className="flex flex-col gap-2">
                  {routes.map((route) => (
                    <div
                      key={route.name}
                      className="rounded-lg border border-[#1a1a1a] bg-[#0d0d0d] p-3 hover:border-[#242424] transition-colors"
                    >
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <span className="text-xs text-[#c0c0c0] leading-snug font-medium">
                          {route.name}
                        </span>
                        <span
                          className={`shrink-0 text-2xs px-1.5 py-0.5 rounded font-medium ${
                            route.status === "on-time"
                              ? "bg-[#0d1f0d] text-[#6fcf73]"
                              : "bg-[#2a1010] text-[#e07070]"
                          }`}
                        >
                      ...
                      <div className="mt-2 h-1 bg-[#1a1a1a] rounded-full overflow-hidden">
                        <div
                          className="h-full rounded-full bg-[#4caf50] opacity-60"
              ...
              <div className="bg-[#0d1f0d] border border-[#1a3a1a] rounded-lg p-4">
                ...
                <div className="text-2xl font-semibold text-[#6fcf73] tracking-tight tabular-nums">
                  −14.2t
                </div>
